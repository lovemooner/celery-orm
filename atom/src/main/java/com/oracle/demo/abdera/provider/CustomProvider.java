package com.oracle.demo.abdera.provider;
...
public class CustomProvider extends AbstractWorkspaceProvider {
	...
	public CustomProvider() {
		...
		addFilter(new OpenSearchFilter().setShortName("My OpenSearch").setDescription("My Description")
				.setTags("test", "example", "opensearch").setContact("john.doe@example.org")
				.setTemplate(
						"http://localhost:8080/atom/feed?q={searchTerms}&c={count?}&s={startIndex?}&p={startPage?}&l={language?}&i={indexEncoding?}&o={outputEncoding?}")
				.mapTargetParameter("q", "searchTerms").mapTargetParameter("c", "count")
				.mapTargetParameter("s", "startIndex").mapTargetParameter("p", "startPage")
				.mapTargetParameter("l", "language").mapTargetParameter("i", "inputEncoding")
